// Cleanup script for master engagement sheet
...
async function cleanupMasterSheet() {
  try {
    ...
    console.log('Current tabs:', existingSheets.map(sheet => sheet.properties.title))
    
    // Find tabs to delete (Mar 31 onwards test tabs)
    const tabsToDelete = existingSheets.filter(sheet => {
      const title = sheet.properties.title
      return title.includes('Mar') || title.includes('Apr') || title.includes('May')
    })
    
    if (tabsToDelete.length === 0) {
      console.log('No test tabs to delete')
      return
    }
    
    console.log('Deleting tabs:', tabsToDelete.map(sheet => sheet.properties.title))
    
    // Create batch delete requests
    const deleteRequests = tabsToDelete.map(sheet => ({
      deleteSheet: {
    ...
    console.log(`✅ Successfully deleted ${tabsToDelete.length} test tabs`)
